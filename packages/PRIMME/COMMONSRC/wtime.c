...
// [RMS: windows version using queryperformancecounter (which sometimes doesn't work! nice!)
double primme_wTimer(int zeroTimer) {
   ...
   if (zeroTimer) {
	...
	LARGE_INTEGER tCur, freq;
	QueryPerformanceCounter(&tCur);
	QueryPerformanceFrequency(&freq);
	return (double)( (tCur.QuadPart - tStart.QuadPart)) / (double)freq.QuadPart;
   }
}


double primme_wTimer2(int zeroTimer) {
   static LARGE_INTEGER tStart;
   static double StartingTime;
...
double primme_wTimer(int zeroTimer) {
   ...
   if (zeroTimer) {
      ...
      gettimeofday(&tv, &tz); 
      return ((double) tv.tv_sec) + ((double) tv.tv_usec ) / (double) 1E6 - StartingTime;
   }
}
*/
